...
# FIX: Fill NaN in first year (no previous year to compare against)
oil_yearly['oil_price_shock_pct'] = oil_yearly['oil_price_shock_pct'].fillna(0)
...
# Optional: Missing year gap check (good for viva)
all_years = set(range(master['year'].min(), master['year'].max() + 1))
present_years = set(master['year'])
missing_years = all_years - present_years
print(f"\n  Missing years check: {sorted(missing_years) if missing_years else 'None — dataset is continuous'}")
